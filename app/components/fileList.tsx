...
export default function FileList({ item, trash, clickedFavourite, inFavourite }: any) {
  ...
  return (
    <div className="w-full px-[14px] py-[10px] flex justify-between cursor-pointer items-center hover:bg-[rgba(125,211,252,0.2)]">
      <div className="self-center w-[95%] flex gap-2 items-center" onDoubleClick={() => window.open(item.link, '_blank')}>
        <Image loader={imageLoader} width={28} height={28} src={item.imageURL} alt="File" className=" rounded-[50%]" />
      ...
      {inFavourite !== 'Yes' && (
        <div className="relative self-center items-center">
          <button className="border-none bg-transparent" onClick={() => toggleFileMenu(item.id)}>
            <FontAwesomeIcon icon={faEllipsis} className="w-3 h-3" />
          </button>
          <div
            style={{ display: 'none' }}
            id={item.id}
            className="absolute right-0 z-50 top-[30px] bg-white border border-solid border-black/10 w-44 h-auto rounded shadow-[0px_1px_2px_rgba(0,0,0,0.1)] flex flex-col"
          >
            <button
            ...
            <button
              className="w-full border-none flex justify-between bg-transparent py-1.5 text-left hover:bg-[rgba(0,0,0,0.1)] px-3 text-xs font-medium text-[rgba(0,0,0,0.8)]"
              onClick={() => {
                toggleFileMenu(item.id);
                const promise = storage.getFileDownload('64748172a5b0bd8409dd', item.id);
                window.open(promise.href, '_blank');
              }}
            >
              Download File
              <FontAwesomeIcon icon={faDownload} className="w-3 h-3 text-[rgba(0,0,0,0.8)]" />
            </button>
            ...
            >
              Delete File
              <FontAwesomeIcon icon={faTrashCan} className="w-3 h-3 text-[rgba(0,0,0,0.8)]" />
            </button>
            <button
            ...
            >
              {item.isFavourite === 'Yes' ? 'Remove from Favourite' : 'Add to favourite'}
              <FontAwesomeIcon icon={faHeart} className="w-3 h-3 text-[rgba(0,0,0,0.8)]" />
            </button>
          </div>
        </div>
      )}
