...
export default function FolderModal({ createFolder, canceled, email }: any) {
  ...
  const checkSiblings = () => {
    return sibilings.find((item: any) => item.folder[0] === folderName) ? false : true;
  };
  ...
  useEffect(() => {
    ...
    promise.then(
      (response) => setSiblings(response.documents.filter((item) => item.folder[1] === pathname && item.folder[2] === email)),
      (err) => console.log(err),
    );
  }, [pathname, email]);
  ...
  return (
    <div className="w-full z-50 h-screen absolute top-0 left-0 flex justify-center items-center bg-[rgba(0,0,0,0.1)]">
      ...
      >
        <p className="text-xs text-[rgba(0,0,0,0.8)] font-medium">Folder Name</p>
        <input
          type="text"
